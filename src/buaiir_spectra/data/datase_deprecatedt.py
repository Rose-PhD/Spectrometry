...
class SpectralDataset_v2(Dataset):
    def __init__(self, data_path, device: Device, label_extra_week: int= 9) -> None:
        ...
        # populate the combined label
        label = self.meta_data.loc[:, 'search_label']
        self.meta_data['combined_label'] = label
    ...
    def populate_data_by_week_BIO_SCEINCE(self, parent_dir, specimen_data):
        ...
        Target how this function extracts the data
    ...
    @staticmethod
    def _rename_df(df: pd.DataFrame, new_col_names: List[str] = ['plant_number', 'score', 'titer_1', 'l_1', 'titer_2', 'l_2', 'titer_3', 'l_3']):
    ...
    def _get_expert_files_CASSAVA_CMD(self, df_path:str):
        ...
        new_col_names = ['plant_number', 'score', 'titer_1', 'l_1' ,'titer_2','l_2', 'titer_3', 'l_3', 'week']
    ...
    def _get_exper_files_RUST(self, df_path: str) -> pd.DataFrame:
        """
        Arranges data for the rust dataframe

        Arg:
            df_path: root to where the file is stored
        
        Return:
            None
        """
        rust_df = pd.read_excel(df_path, engine="openpyxl")
        week_mask = (
            rust_df.iloc[:, 0]
            .astype(str)
            .str.match(r'^\s*\d+(st|nd|rd|th)\s+Week\s*$', case=False, na=False)
        )
        ...
        rust_df['week'] = rust_df['week'].ffill()
        # remove week headers
        rust_df = rust_df.loc[~week_mask].copy()

        # remove completely empty rows
        rust_df = rust_df.drop(columns=['1st Week'])
        rust_df = rust_df.dropna(
            subset=['Plant 1', 'Plant 2', 'Plant 3'],
            how='all'
        ).reset_index(drop=True)
        rust_df['week'] = rust_df['week'].fillna(1)

        # creating the plant number 
        rust_df['plant_number'] = rust_df.groupby('week').cumcount() + 1
        rust_df['score'] = rust_df[['Plant 1', 'Plant 2', 'Plant 3']].mean(axis=1).astype('int')
        rust_df = rust_df.dropna(axis=1, how='all') # drop all Nan Columns
        rust_df = rust_df.drop(columns=['Plant 1', 'Plant 2', 'Plant 3'])

        # ensure column consistence
        rust_df['l_1'] = np.nan
        rust_df['l_2'] = np.nan
        rust_df['l_3'] = np.nan
        ...
        old_col_names = [
            'Plant description ',
            'BR-rt-PCR Ct value',
            'BR-rt-PCR Ct value.1',
            'BR-rt-PCR Ct value.2',
        ]
        rename_dict = {old_col: new_col for old_col, new_col in zip(old_col_names, new_col_names)}
        rust_df = rust_df.rename(columns=rename_dict)
        # remove description column
        rust_df = rust_df.drop(columns=['description'])

        # attach plant_type and disease_class
        rust_df['disease_class'] = 'BRD'
        # rust_df['plant_type'] = 'B'
        rust_df['week'] = rust_df['week'].astype('int64')
        rust_df['titer_avg'] = rust_df.apply(self.robust_titer_mean, axis=1)

        # computation of the average titer value

        new_order = ['plant_number','score','titer_1','l_1',\
                     'titer_2','l_2', 'titer_3', 'l_3','week',\
                     'disease_class','titer_avg']

        rust_df = rust_df[new_order]
    ...
    def _get_expert_files_CASSAVA_CBB(self, df_path:str):
        ...
        # format column names
        new_col_names = ['plant_number', 'description', 'score' ,'titer_1', 'l_1' ,'titer_2','l_2', 'titer_3', 'l_3', 'week']
    ...
    def populate_expert_readings(self) -> None:
        ...
        for file in self.expert_files:
            if 'cassava' in file and '~' not in file:
                ...
                rust_df = self._get_exper_files_RUST(file)
                expert_dfs.append(rust_df)
    ...
    def read_single_LOW_COST(path: str) -> Tuple[np.ndarray]:
        ...
        try:
            ...
            print(f'Error occured for path: {path}')
            raise
        ...
        if isinstance(spectral_1, dict):
            ...
            # Computation of calibration data
            calibration = ast.literal_eval(df0['calibration'][0])['intensity']
        else:
            spectral_2 = ast.literal_eval(df0['spectral_2'][0])
            band_energy_1 = ast.literal_eval(df0['band_energy_spectral_1'][0])
            band_energy_2 = ast.literal_eval(df0['band_energy_spectral_2'][0])
            calibration = ast.literal_eval(df0['calibration'][0])
            
        
        # Type conversion
        calibration = np.array(calibration, dtype=np.float32)
        spectral_1 = np.array(spectral_1, dtype=np.float32)
        spectral_2 = np.array(spectral_2, dtype=np.float32)
        band_energy_1 = np.array(band_energy_1, dtype=np.float32)
        band_energy_2 = np.array(band_energy_2, dtype=np.float32)

        # calibration computation
        EPS = 1e-6
        EPS_1 = np.mean(spectral_1) / np.maximum(np.mean(calibration), EPS)
        EPS_2 = np.mean(spectral_2) / np.maximum(np.mean(calibration), EPS)


        spectral_1 = (spectral_1 + calibration) / np.maximum(spectral_1 - calibration, EPS_1)
        spectral_2 = (spectral_2 + calibration) / np.maximum(spectral_2 - calibration, EPS_2)

        # spectral_1 = (spectral_1  + calibration)/ EPS_1
        # spectral_2 = (spectral_2 + calibration) / EPS_2
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray]:
        ...
        if self.device == Device.BIO_SCIENCE:
            ...
            # band_energy = 6
            band_energy = 0 # eliminate band energy for now
        ...
        y_out = np.zeros(N_titer_readings + Disease_class_size + week_size + expert_score_size + plant_type_size)

        # Fill in the target data
        target_cols = (['titer_avg','score', 'plant_type', 'week', 'disease_class'])
        ...
        for i in range(n_cols):
            if i == n_cols - 1:
                ...
                y_out[i] = self.plant_type_codes[self.meta_data.loc[index, target_cols[i]]]
            else:
        ...
        for i, file in enumerate(self.read_one(index)):
            if self.device == Device.SCAN_CODER:
                ...
                spectral_1, spectra_2, _, _ = file
                temp_buffer.extend([spectral_1, spectra_2])
...
class SpectralDataset(SpectralDataset_v2):
    ...
    def __init__(self, data_path: str, device: Device, label_extra_week: int=9, load_with_images=False):
        """
        Arg:
    ...
    @staticmethod
    def _load_images(image_list: List):
        """
        Loads images of a single index 

        Arg:
            image_list: list containing all images

        Return:
            generator about the image
        """
    ...
    def __getitem__(self, index: int):
        """
        Loads data for the target device

        Arg:
            index: int -> position of data to be loaded
        
        Return:
            Tuple[np.ndarry] -> contains only (x, y) with_load_images= False, and (x, y, image) with load_images=True 
        """
        x, y = super().__getitem__(index)
        out = [x, y]

        if self.load_with_images:
            image_dirs = self.meta_data.loc[index, 'img_data_dirs']

            image_buffer = []
            for file in self._load_images(image_dirs):
                image_buffer.append(file)

            image_buffer = np.array(image_buffer)

            n_data = x.shape[0]
            n_images, h,w , c = image_buffer.shape
            diff = n_data - n_images

            if diff > 0:
                idx = np.random.choice(n_images, size=diff)
            
            
                out_images = np.zeros((n_data, h, w, c))
                out_images[:n_images] = image_buffer

                for i, pos in zip(range(n_images, n_data), idx):
                    out_images[i] = image_buffer[pos]
                
                image_buffer = np.array(out_images)
                out.append(image_buffer)
            else:
                # read a few images
                sample_images = image_buffer[:n_data]
                out.append(sample_images)

        # compute the labels
        labels = self.meta_data.loc[index, 'combined_label']
        out.append([labels] * out[0].shape[0])
        
        if self.load_with_images:
            return out[0], out[-2], out[1], out[-1]
        else:
            return out
...
if __name__ == '__main__':
    ...
    dataset = SpectralDataset(data_path, Device.BIO_SCIENCE, load_with_images=False)
    ...
    filtered = dataset.meta_data[dataset.meta_data['disease_class'] == disease_class].reset_index(drop=True)
    print(filtered.head(60))
    print(filtered.tail(20))

    # x, y, image = dataset[12]
